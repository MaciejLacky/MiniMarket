...
{
    public class NowyTowarEditViewModel : JedenViewModel<Towary>, IDataErrorInfo
    {
        #region Fields
        ...
        #endregion
        #region Constructor
        public NowyTowarEditViewModel(int wybraneId)
        {
            base.DisplayName = "Nowy towar";
            this._WybraneId = wybraneId;
            item = miniMarketEntities.Towary.Where(x => x.Twr_IdTowaru == wybraneId).FirstOrDefault();
        ...
        public decimal? CenaZakupuBrutto
        {
            get
            {
                return item.Twr_CenaZakBrutto = CenaZakupuNetto * (IdVatZakup == null ? (VatZakupComboBoxItem.First().VatZak_Wartosc / 100 + 1) : VatZakupComboBoxItem.FirstOrDefault(x => x.VatZak_Id == IdVatZakup).VatZak_Wartosc / 100m + 1m);
            }
            set
            {
        ...
        public decimal? CenaSprzedazyBrutto
        {
            get
            {
                return item.Twr_CenaSprBrutto = CenaZakupuNetto * (Marza / 100m + 1m) * (IdVatSprzedaz == null ? (VatSprzedazComboBoxItem.First().VatSpr_Wartosc / 100m + 1m) : VatSprzedazComboBoxItem.FirstOrDefault(x => x.VatSpr_Id == IdVatSprzedaz).VatSpr_Wartosc / 100m + 1m);
            }
            set
            {
        ...
        public override void Save()
        {
            ...
            item.Twr_ZmiUzytData = DateTime.Now;

            miniMarketEntities.Towary.FirstOrDefault(x => x.Twr_IdTowaru == _WybraneId).Twr_Kod = Kod;
            miniMarketEntities.Towary.FirstOrDefault(x => x.Twr_IdTowaru == _WybraneId).Twr_Nazwa = Nazwa;
            miniMarketEntities.Towary.FirstOrDefault(x => x.Twr_IdTowaru == _WybraneId).Twr_NumerKat = NumerKatalogowy;
            miniMarketEntities.Towary.FirstOrDefault(x => x.Twr_IdTowaru == _WybraneId).Twr_EAN = Ean;
            miniMarketEntities.Towary.FirstOrDefault(x => x.Twr_IdTowaru == _WybraneId).Twr_Typ = Typ;
            miniMarketEntities.Towary.FirstOrDefault(x => x.Twr_IdTowaru == _WybraneId).Twr_PKWiU = PKWiU;
            miniMarketEntities.Towary.FirstOrDefault(x => x.Twr_IdTowaru == _WybraneId).Twr_IdVatSprzedaz = IdVatSprzedaz;
            miniMarketEntities.Towary.FirstOrDefault(x => x.Twr_IdTowaru == _WybraneId).Twr_IdVatZakup = IdVatZakup;
            miniMarketEntities.Towary.FirstOrDefault(x => x.Twr_IdTowaru == _WybraneId).Twr_CenaDomysl = IdCenaDomyslna;
            miniMarketEntities.Towary.FirstOrDefault(x => x.Twr_IdTowaru == _WybraneId).Twr_CenaZakNetto = CenaZakupuNetto;
            miniMarketEntities.Towary.FirstOrDefault(x => x.Twr_IdTowaru == _WybraneId).Twr_CenaZakBrutto = CenaZakupuBrutto;
            miniMarketEntities.Towary.FirstOrDefault(x => x.Twr_IdTowaru == _WybraneId).Twr_CenaSprNetto = CenaSprzedazyNetto;
            miniMarketEntities.Towary.FirstOrDefault(x => x.Twr_IdTowaru == _WybraneId).Twr_CenaSprBrutto = CenaSprzedazyBrutto;
            miniMarketEntities.Towary.FirstOrDefault(x => x.Twr_IdTowaru == _WybraneId).Twr_Waluta = Waluta;
            miniMarketEntities.Towary.FirstOrDefault(x => x.Twr_IdTowaru == _WybraneId).Twr_Marza = Marza;
            miniMarketEntities.Towary.FirstOrDefault(x => x.Twr_IdTowaru == _WybraneId).Twr_IdJednostkaPodstawowa = IdJednostkaPodstawowa;
            miniMarketEntities.Towary.FirstOrDefault(x => x.Twr_IdTowaru == _WybraneId).Twr_IdJednostkaPomocnicza = IdJednostkaPomocnicza;
            miniMarketEntities.Towary.FirstOrDefault(x => x.Twr_IdTowaru == _WybraneId).Twr_IdKategoria = IdKategoriiSpr;
            miniMarketEntities.Towary.FirstOrDefault(x => x.Twr_IdTowaru == _WybraneId).Twr_IdKategoria = IdKategoriiZak;
            miniMarketEntities.Towary.FirstOrDefault(x => x.Twr_IdTowaru == _WybraneId).Twr_IdGrupa = IdGrupa;
            miniMarketEntities.Towary.FirstOrDefault(x => x.Twr_IdTowaru == _WybraneId).Twr_IdTypTowaru = IdTypTowaru;
        ...
        {
            if (this["Nazwa"] == null && this["Kod"] == null)
            {
                return true;
            }
            return false;
